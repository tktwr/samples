...
void f_rect(tt::Image<tt::RGBA8>& image, const std::array<int, 2>& o, const std::array<int, 2>& size, const tt::RGBA8& color) {
    image.foreach([&](tt::RGBA8& val, int x, int y) {
...
void f_create_vstripe_image(tt::Image<tt::RGBA8>& image, int n, const tt::RGBA8& color0, const tt::RGBA8& color1) {
    int d = image.w() / n;
    int d2 = d * 2;
    image.foreach([&](tt::RGBA8& val, int x, int y) {
        if (x%d2 < d) {
...
void f_create_hstripe_image(tt::Image<tt::RGBA8>& image, int n, const tt::RGBA8& color0, const tt::RGBA8& color1) {
    int d = image.h() / n;
    int d2 = d * 2;
    image.foreach([&](tt::RGBA8& val, int x, int y) {
        if (y%d2 < d) {
...
void f_image_to_cvmat(const tt::Image<tt::RGBA8>& image, cv::Mat& mat) {
    mat = cv::Mat(image.h(), image.w(), CV_8UC4, (tt::RGBA8*)(image.data()));
}
